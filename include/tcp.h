...
class TCP_Fully_Connected {
 public:
  ...
  void init() {
    ...
    if (!val) LOGGING_FATAL("Could not read USER env variable.");
    std::string env_path = "/home/" + std::string(val) + "/sunlab.env";

    std::fstream file(env_path, std::ios::in);
    if (!file.is_open()) LOGGING_FATAL("Failed to open env file at path: {}", env_path);
    ...
    while (std::getline(buf, line)) {
      ...
      if (std::find(cfg_.nodes.begin(), cfg_.nodes.end(), id) == cfg_.nodes.end()) continue;
      ...
      if (getaddrinfo(host.c_str(), std::to_string(cfg_.port).c_str(), &hints, &result) != 0)
        LOGGING_FATAL("Failed to resolve hostname: {}", host);

      for (int i = 0; i < cfg_.conns_per_node; ++i) {
        int fd = socket(AF_INET, SOCK_STREAM, 0);
        if (fd < 0) LOGGING_FATAL("Failed to create socket: {}", strerror(errno));
    ...
    int expected = static_cast<int>((cfg_.system_size - 1) * cfg_.conns_per_node);
    while (num_conns_.load() < expected) {
      LOGGING_DEBUG("Waiting for connections... {}/{}", num_conns_.load(), expected);
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
 ...
 private:
  void start_server() {
    ...
    if (server_fd < 0) LOGGING_FATAL("Failed to create server socket: {}", strerror(errno));
